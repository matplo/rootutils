...
def load_lib(libpath, silent=True):
...
def __h1d_from_ntuple(fname, ntname, var, cuts, bwidth, xlow, xhigh, title=None, modname='', nev=-1):
    ...
    try:
        ...
        print('[e] draw from ntuple failed [{}:{}:{}]'.format(fname, ntname, var), file=sys.stderr)
    return hret
...
def get_max_from_ntuple(fname, ntname, var, cuts = None):
    ...
    print('[e] unable to get maximum of',var,'from',fname,ntname, file=sys.stderr)
    return None
...
def get_mean_from_ntuple(fname, ntname, var, cuts = ''):
    ...
    if fin:
        ...
        if tn:
            ...
            print('*** mean = ', mean)
            return mean
    print('[e] unable to get mean of',var,'from',fname,ntname, file=sys.stderr)
    return None
